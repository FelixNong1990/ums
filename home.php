<?php include_once('header.php'); ?>
<?php include_once('classes/check.class.php'); ?>
<?php include_once('classes/index.php');?>

<?php 
//echo $_SESSION['jigowatt']['user_level'][0]; 
//echo $_SESSION['jigowatt']['user_id'];
?>
 <?php protect("*"); ?>
<!--
<div class="hero-unit">
	<h1><?php _e('Let\'s talk about the future.'); ?></h1>
	<h2><?php _e('Easy user management and a comprehensive admin panel. All, for you :3'); ?></h2>
	<p>
		<a href="http://goo.gl/94EYi" target="_TOP" class="btn btn-info btn-large"><?php _e('Purchase this script'); ?> &raquo;</a>
		<a href="http://eepurl.com/jNgF9" class="btn btn-large" target="_blank"><?php _e('Subscribe to updates'); ?></a>
	</p>
	<p class="info-links">
		<a href="http://jigowatt.co.uk/themeforest/login/install.php" target="_blank"><?php _e('Documentation'); ?></a>
		<a href="http://codecanyon.net/item/php-login-user-management/discussion/49008" target="_blank"><?php _e('Support'); ?></a>
	</p>
</div>

<hr>
-->
<div class="features">
	<table class="address_listing table table-striped table-bordered">
		<thead>
			<tr>
				<th>Case Number</th>
				<th>Respondent Name</th>
				<th>Block</th>
				<th>Road Name</th>
				<th>Unit No</th>
				<th>Postal code</th>
				<th>Age Group</th>
				<th>Gender</th>
				<th>Nationality</th>
				<th>Visit Count</th>
				<th>Status</th>
				<th>Remarks</th>
			</tr>
		</thead>
    <tbody>
<?php 
	$row = $index->getAl($_SESSION['jigowatt']['user_id']);
	// print "<pre>";
	// print_r($row);
	// print "</pre>";
	
	foreach($row as $result):
?>
    <tr>
		<td><?php echo $result['case_number']; ?></td>
		<td><?php echo $result['respondent_name']; ?></td>
		<td><?php echo $result['block']; ?></td>
		<td><?php echo $result['road']; ?></td>
		<td><?php echo $result['unit']; ?></td>
		<td><?php echo $result['postal_code']; ?></td>
		<td><?php echo $result['age']; ?></td>
		<td>
			<?php 
			if($result['gender'] == 0) echo "Male";
			else echo "Female";
			?>
		</td>
		<td><?php echo $result['nationality']; ?></td>
		<td><?php echo $result['visit_count']; ?></td>
		<td><?php echo $result['visit_status']; ?></td>
		<td><?php echo $result['remarks']; ?></td>
    </tr>
<?php endforeach; ?>	
	</tbody>
    </table>
	<!--<div class="row">
		<h1><?php _e('Stupendously exciting login and user management.'); ?></h1>
		<p class="intro"><?php _e('Just the right amount of tools to get your job done.'); ?></p>
		<div class="span6">
			<h2><?php _e('Installation'); ?>
				<?php if (file_exists('classes/config.php')) : ?>
				<small><?php _e('Complete'); ?></small>
				<?php else : ?>
				<small><?php _e('Not complete'); ?></small>
				<?php endif; ?>
			</h2>
			<p><?php _e('Get setup in minutes! Enjoy the super easy installation wizard to walk you through the setup process.'); ?></p>
			<?php if (!file_exists('classes/config.php')) : ?>
			<p><?php _e('Start your installation by clicking the button below!'); ?></p>
			<p><a href="install/index.php" class="btn btn-success"><?php _e('Begin Install'); ?></a></p>
			<?php endif; ?>
		</div>

		<div class="span6">
			<h2><?php _e('Reports'); ?></h2>
			<p><?php _e('Keep track of how your site is doing with dynamic graphs to aid you.'); ?></p>
		</div>
	</div>

	<div class="row">
		<div class="span6">
			<h2><?php _e('Security'); ?></h2>
			<p><?php _e('Our script has a couple default secure pages to get you started.'); ?></p>
		</div>

		<div class="span6">
			<h2><?php _e('Admin tools'); ?></h2>
			<p><?php _e('Enjoy the luxury of having control over every aspect of your website.'); ?></p>
		</div>
	</div>-->
</div>

	<br><br><hr>

<!--<div class="demo features">
	<div class="row">
		<h1><?php _e('Demo credentials.'); ?></h1>
		<p class="intro"><?php _e('Whatcha waiting for? Login and check out the site!'); ?></p>

		<div class="span3 offset2">
			<h2><?php _e('Administrator'); ?></h2><br>
			<code><?php _e('Username: admin'); ?></code><br>
			<code><?php _e('Password: admin'); ?></code>
		</div>

		<div class="span3">
			<h2><?php _e('Special user'); ?></h2><br>
			<code><?php _e('Username: special'); ?></code><br>
			<code><?php _e('Password: special'); ?></code>
		</div>

		<div class="span3">
			<h2><?php _e('Default privileges'); ?></h2><br>
			<code><?php _e('Username: user'); ?></code><br>
			<code><?php _e('Password: user'); ?></code>
		</div>
	</div>
</div>-->

<?php include_once('footer.php'); ?>