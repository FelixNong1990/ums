...
class Address_Listings extends Generic {
	...
	private function validate() {
		...
		if (empty($this->settings['case_number']))
				$this->error .= '<div class="alert alert-error">'._('You must enter a case number.').'</div>';
		
		if (empty($this->settings['block']))
				$this->error .= '<div class="alert alert-error">'._('You must enter a block.').'</div>';
	...
	private function process() {
		$surveyor_id = $this->user_id;
		//echo $this->user_id;
		$params = array (
			//':username' => $this->username,
			':surveyor_id' => $surveyor_id,
			':case_number'     => $this->settings['case_number'],
			':respondent_name'     => $this->settings['respondent_name'],
			':block'     => $this->settings['block'],	
			':road'     => $this->settings['road'],
			...
			':visit_count'     => $this->settings['visit_count'],
			':visit_status'     => $this->settings['visit_status'],
			':remarks'     => $this->settings['remarks']
		);
		$sql = "INSERT INTO `login_address_listings` 
		(`surveyor_id` , `case_number` , `respondent_name` , `block` , `road` , `unit` , `postal_code`,
		`age` , `gender` , `nationality` , `visit_count` , `visit_status` , `remarks`
		) 
		VALUES (:surveyor_id , :case_number , :respondent_name , :block , :road , :unit , :postal_code,
		:age , :gender , :nationality , :visit_count , :visit_status , :remarks
		)";
		parent::query($sql,$params);

		//$this->error = "<div class='alert alert-success'>"._('User information updated for')." <b>".$this->settings['name']."</b> ($this->username).</div>";
		$this->error = "<div class='alert alert-success'>"._('New address listing has been success fully added.')."</div>";
